  ...
  if (sections.length === 1 && !sections[0].name) {
      ...
      >
        <ReactMarkdown
          ...
          h2: ({ children }) => <h2 className="text-term-600 dark:text-term-400 font-bold font-serif" style={{ marginTop: `${paragraphSpacing}rem`, marginBottom: `${paragraphSpacing * 0.5}rem` }}>{children}</h2>,
          code: ({ node, inline, className, children, ...props }) => {
            ...
            return !inline ? (
              <pre className="bg-stone-200 dark:bg-stone-900 p-4 rounded-md overflow-x-auto whitespace-pre-wrap break-all w-full" style={{ marginTop: '0', marginBottom: `${paragraphSpacing}rem` }}>
                <code className={`${match ? `language-${match[1]}` : ''} font-mono block text-gray-800 dark:text-term-100`} {...props}>
                  {children}
                </code>
              </pre>
            ) : (
          ...
          p: ({ children }) => (
            <p className="whitespace-pre-wrap font-serif w-full text-gray-800 dark:text-term-100" style={{ marginBottom: `${paragraphSpacing}rem` }}>
              {children}
            </p>
          ),
          ul: ({ children }) => (
            <ul 
              className="list-disc pl-4 w-full text-gray-800 dark:text-term-100"
              style={{ marginTop: '0', marginBottom: `${paragraphSpacing}rem` }}
            >
              {children}
            </ul>
          ),
          li: ({ children }) => <li className="text-gray-800 dark:text-term-100">{children}</li>,
        }}
              >
          {content}
        </ReactMarkdown>
    ...
    <div className="text-left font-serif w-full">
      {sections.map((section, index) => (
        <div key={index} className={index > 0 ? 'mt-6' : ''}>
          ...
          <div 
            className="text-gray-800 dark:text-term-100 markdown-content"
            style={{ '--paragraph-spacing': `${paragraphSpacing}rem` }}
          >
            <ReactMarkdown
              className="text-left font-serif w-full"
