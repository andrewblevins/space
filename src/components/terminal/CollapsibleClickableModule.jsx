...
export function CollapsibleClickableModule({ title, items = [], expanded, onToggle, onItemClick }) {
  return (
    <div className="bg-gray-900 p-4">
      <div className="flex justify-between items-center cursor-pointer hover:text-orange-300 mb-2" onClick={onToggle}>
      ...
      {expanded && (
        <ul className="space-y-4">
          {items.map((item, idx) => (
            <li
              key={idx}
              className="text-gray-300 cursor-pointer hover:text-orange-400 transition-colors whitespace-pre-wrap"
