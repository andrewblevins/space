    ...
    <div className="bg-gray-900 p-4">
      ...
      {expanded && (
        <ul className="space-y-4">
          {items.map((item, idx) => (
            <li key={idx} className="flex items-center justify-between text-gray-300 hover:text-green-400 transition-colors">
              <span className="whitespace-pre-wrap">{item}</span>
              <button onClick={() => onItemClick && onItemClick(item)} className="ml-2 text-green-400 hover:text-green-300 transition-colors">
